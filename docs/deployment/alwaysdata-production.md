...
and the baseline for the automated deployment pipeline defined in
`docs/specs/SPEC-023-automated-production-deployment.md` and implemented at
`.github/workflows/deploy-production.yml` (see Section 21).
...
exists **only** as a production secret in the server `.env`; the automated
pipeline (Section 21) never stores `DB_PASSWORD` or `APP_KEY` in GitHub.
...
The implemented automated pipeline (Section 21) also does **not** use
`rsync --delete`. `--delete` may only be enabled with explicit exclusions that
...
## 21. Automated CI/CD deployment (implemented)

Implemented per `docs/specs/SPEC-023-automated-production-deployment.md`.
...
Trigger: `push` to `main` (approved PR merges included) plus `workflow_dispatch`
as manual emergency/retry. Never runs on feature/fix/docs branches.
...
→ Frontend quality gate (build artifact public/build uploaded)
→ both pass?
→ Deploy job (GitHub Environment production)
→ SSH alwaysdata (dedicated key + verified known_hosts)
→ safe rsync (no --delete)
→ composer install --no-dev + check-platform-reqs
→ php artisan about verification (APP_ENV=production, APP_DEBUG=OFF)
→ php artisan migrate --force
→ php artisan optimize + view:cache
→ chmod -R u+rwX storage bootstrap/cache
→ HTTP health checks (bounded retries)
→ deployment record → storage/logs/deployment.log
→ GITHUB_STEP_SUMMARY
```

### Required GitHub secrets

In the GitHub Environment named `production`:

```text
ALWAYSDATA_HOST=ssh-yaris.alwaysdata.net
ALWAYSDATA_USER=yaris
ALWAYSDATA_PORT=22
ALWAYSDATA_SSH_KEY (dedicated CI private key, secret only)
ALWAYSDATA_KNOWN_HOSTS (verified host key line)
```

Optional overrides: `ALWAYSDATA_APP_ROOT` (default
`/home/yaris/apps/agenda-estetica`) and `ALWAYSDATA_BASE_URL` (default
`https://yaris.alwaysdata.net`).

`DB_PASSWORD`, `APP_KEY` and the admin SSH key are **never** stored in GitHub.

### rsync exclusions

The workflow syncs the repository root into `/home/yaris/apps/agenda-estetica/`
and always excludes:

```text
.git/  node_modules/  vendor/  .env  .env.*  .github/
docs/  tests/  coverage/  .phpunit.cache/  storage/
bootstrap/cache/  public/hot  .gitignore  .editorconfig
.DS_Store  AGENTS.md  README.md
```

`public/build` (the CI-built frontend) **is** deployed. `vendor` is installed
server-side from `composer.lock`.

### Activation

Before the pipeline can deploy, run the steps in the SPEC-023
`Activation checklist`: generate the dedicated ED25519 CI key
(`~/.ssh/id_ed25519_alwaysdata_github`), authorize its public key in alwaysdata,
create the `production` Environment and the 5 secrets above, then trigger a
manual `workflow_dispatch` run on a clean `main` and confirm SSH connectivity
and health checks pass.

### Failure behavior

- Any quality gate failure → deploy job never starts.
- Missing secrets or connectivity problems → deploy fails fast with a clear
  error in the job log.
- Migration failure or failed health check after deploy → the workflow reports
  **failed**; the deployment is not reported as success.

### Rollback limitation

The automated pipeline is rsync-in-place and therefore **not atomic**: there is
no automatic rollback and no release-directory switching yet. Recovery after a
bad deploy is manual (restore the previous clean SHA following the manual
deployment policy of SPEC-023). This limitation is intentional and documented in
SPEC-023; reuse its rollback analysis before calling the pipeline
production-grade.