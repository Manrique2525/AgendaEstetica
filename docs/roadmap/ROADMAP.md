...
`SPEC-024 - Fiscal Demo & Consent UX` está en estado `DEVELOPMENT IN PROGRESS / CHECKPOINT A IMPLEMENTED / READY FOR HUMAN APPROVAL` en `docs/specs/SPEC-024-fiscal-demo-consent-ux.md`. Checkpoint A cubre únicamente la UX compacta de consentimiento, su ciclo `sessionStorage` y la accesibilidad; Checkpoint B no está autorizado. La demo fiscal, XML, CFDI, SAT/PAC y firma digital permanecen pendientes; no reabre SPEC-022, no modifica SPEC-023 y no despliega.
