...
describe('DemoOrderPage', () => {
    ...
    it('shows the static educational PDF only when the demo invoice is requested', async () => {
        const wrapper = mount(DemoOrderPage);
    ...
    it('keeps the privacy notice and integrity payload visible in the final state', async () => {
        const wrapper = mount(DemoOrderPage);
        ...
        expect(wrapper.text()).toContain('No se creó un pedido real');
    });
});
